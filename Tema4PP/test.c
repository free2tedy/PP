...
struct test bonus_tests[] = {
	{1, {"tests/t8.txt", NULL}},
	{1, {"tests/t19.txt", NULL}},
	{1, {"tests/t26.txt", NULL}},
	{1, {"tests/t11.txt", NULL}},
	{2, {"tests/t8.txt", "tests/t19.txt", NULL}},
	{2, {"tests/t8.txt", "tests/t26.txt", NULL}},
	{2, {"tests/t8.txt", "tests/t11.txt", NULL}},
	{5, {"tests/t19.txt", "tests/t26.txt", "tests/t11.txt", NULL}},
	{5, {"tests/t8.txt", "tests/t19.txt", "tests/t26.txt", "tests/t11.txt", NULL}},
};
#define NUM_BONUS_TESTS 9
...
char *bonus_test_names[] = {"lost", "city2", "broken", "sand-storm", "promises",
	"dreams", "loop3", "round1"};
#define BONUS_TESTS 8

#define INITIAL_FACT(f) (strcmp(f->name, "initial-fact") == 0)
#define WIN_FACT(f) (strcmp(f->name, "win") == 0)
#define NEED_FACT(f) (strcmp(f->name, "need") == 0)
...
/* unneeded need fact */
#define UNNEEDED_NEED 51
/* missing need fact */
#define NO_NEEDED_NEED 52
...
int check_for_initial_fact(struct fact_table *ft, void *unused, int unusedint)
{
	unused = unused;
	unusedint = unusedint;
...
int check_for_extra_fact(struct fact_table *ft, void *unused, int unusedint)
{
	unused = unused;
	unusedint = unusedint;

	while (ft)
		if (!INITIAL_FACT(ft->fact) && !WIN_FACT(ft->fact) && !NEED_FACT(ft->fact))
			return EXTRA_FACTS;
		else
			ft = ft->next;
...
int check_unknown_id(struct fact_table *ft, struct world_list *worlds)
{
	...
	for (; ft; ft = ft->next) {
		found = 0;
		if (WIN_FACT(ft->fact))
			wid = ((struct win_record*)ft->fact->data)->id;
		else if (NEED_FACT(ft->fact))
			wid = ((struct need_record*)ft->fact->data)->id;
		else
			continue;

		for (w = worlds; w; w = w->next) {
			if (strcmp(w->world->id, wid))
				continue;
			found = 1;
			break;
		}
...
int check_incorrect(struct fact_table *ftbl, struct world_list *wl)
{
	...
	for (ft = ftbl; ft; ft = ft->next) {
		...
		/* found a winning fact but the world was not winnable from
		 * the start
		 */
		if (!found) {
			for (i = 0; i < BONUS_TESTS; i++) {
				if (strcmp(bonus_test_names[i], wr->id))
					continue;
				goto next_check;
			}
...
int valid_path(struct fact_table *ftbl, struct world_list *wlst, int bonus)
{
	struct fact_table *ft;
	struct need_record *nr;
	struct win_record *wr;
	struct world_list *wl;
	int found;

	/* check that all worlds in wl have a need */
	for (wl = wlst; wl; wl = wl->next) {
		found  = 0;

		for (ft = ftbl; ft; ft = ft->next) {
			if (!NEED_FACT(ft->fact))
				continue;

			nr = ft->fact->data;

			if (strcmp(nr->id, wl->world->id))
				continue;

			/* place need */
			wl->world->world[nr->x][nr->y] = MAN;
			found = 1;
			break;
		}

		if (!found && bonus)
			return NO_NEEDED_NEED;
...
int test_everything(struct fact_table *ft, void *data, int bonus)
{
	struct world_list *worlds = data;

	if (check_for_initial_fact(ft, worlds, bonus) != TEST_OK)
		return NO_INITIAL_FACT;

	if (check_for_extra_fact(ft, worlds, bonus) != TEST_OK)
		return EXTRA_FACTS;
	...
	return valid_path(ft, worlds, bonus);
}

void test_results(int tid, char *cmd,
		int (*check_result)(struct fact_table *, void *, int),
		void *data,
		int score,
		int bonus)
{
	...
	switch (check_result (ft, data, bonus)) {
	...
	case DUPLICATE_FACTS:
		...
		goto out;
	case UNNEEDED_NEED:
		printf("..(unneeded need found).......................failed [  0 ]\n");
		goto out;
	case NO_NEEDED_NEED:
		printf("..(needed need fact not found)................failed [  0 ]\n");
		goto out;
	}
...
void test(int ix, int bonus) {
	struct world_list *wl = NULL, *tmp;
	struct test test = bonus ? bonus_tests[ix] : all_tests[ix];
	char *cmd;
	int i;
	...
	test_results((bonus ? 34 : 4) + ix, cmd, test_everything, wl, test.score, bonus); /* syntax check */
...
int main(int argc, char **argv)
{
	...
	if (argc > 1 && sscanf(argv[1], "%d", &i) && i > 0 && i < NUM_TESTS + NUM_BONUS_TESTS + 4) {
		switch (i) {
		case 1: test_results(1, build_initial_cmd(), NULL, NULL, 1, 0);
			break;
		case 2: test_results(2, build_initial_cmd(), check_for_initial_fact, NULL, 1, 0);
			break;
		case 3: test_results(3, build_initial_cmd(), check_for_extra_fact, NULL, 1, 0);
			break;
		default: 
			if (i >= NUM_TESTS + 4)
				test(i - NUM_TESTS - 4, 1);
			else
				test(i - 4, 0);
		}
		return 0;
	...
	test_results(1, build_initial_cmd(), NULL, NULL, 1, 0); /* syntax check */
	test_results(2, build_initial_cmd(), check_for_initial_fact, NULL, 1, 0);
	test_results(3, build_initial_cmd(), check_for_extra_fact, NULL, 1, 0);
...
int parse_need_fact(struct fact *fact)
{
	int retval = 1;
	char *tmp = strtok(NULL, "( )"); /* should be id */
	struct need_record *need = calloc(1, sizeof(*need));
	...
	tmp = strtok(NULL, "( )"); /* should be x */
	if (!tmp)
		goto out;
	tmp = strtok(NULL, "( )");
	if (!tmp || sscanf(tmp, "%d", &(need->x)) != 1)
		goto out;

	tmp = strtok(NULL, "( )"); /* should be y */
	if (!tmp)
		goto out;
	tmp = strtok(NULL, "( )");
	if (!tmp || sscanf(tmp, "%d", &(need->y)) != 1)
		goto out;
...
int parse_fact(const char *s, struct fact *fact)
{
	...
	else if (NEED_FACT(fact))
		retval = parse_need_fact(fact);
	else {
		char *fact_data;
